...
instance PP [Int] where
  pp xs = PP.brackets $ PP.hcat $ PP.punctuate PP.comma $ map pp xs
...
instance PP Var where
  ...
  pp (Proj expr1 expr2) = pp expr1 <> PP.brackets (pp expr2)

-- | TODO: Implement pretty printing for blocks

instance PP Block where
  pp (Block stmts) = PP.lbrace PP.$+$ PP.nest 4 (PP.vcat $ map pp stmts) PP.$$ PP.rbrace

-- | TODO: Implement the rest of pretty printing for statements.

instance PP Statement where
  pp Empty = PP.empty
  pp (Decl block expr) = PP.text "var" <+> pp block <+> PP.text ":=" <+> (pp expr <> PP.semi)
  pp (Assign var expr) = pp var <+> PP.text ":=" <+> (pp expr <> PP.semi)
  pp (Assert p) = PP.text "assert" <+> (pp p <> PP.semi)
  pp (If expr block (Block [])) = PP.empty PP.$$ PP.text "if" <+> pp expr <+> pp block
  pp (If expr block1 block2) = PP.empty PP.$$
    PP.text "if" <+> pp expr <+> pp block1 <+> PP.empty PP.$$
    PP.text "else" <+> pp block2
  pp (While p expr block) =
    PP.text "while" <+> pp expr
    PP.$$ PP.nest 2 (pp p)
...
instance PP Method where
  pp (Method n ins outs specs block) =
    PP.text "method" <+> PP.text n
    <+> PP.parens (PP.hsep $ PP.punctuate PP.comma $ map pp ins)
    <+> PP.text "returns" <+> PP.parens (PP.hsep $ PP.punctuate PP.comma $ map pp outs)
    PP.$$ PP.vcat (map ppSpec specs)
    PP.$$ pp block 
    where
      ppSpec (Requires p) = PP.text "requires" <+> pp p
      ppSpec (Ensures p)  = PP.text "ensures" <+> pp p
      ppSpec (Modifies n) = PP.text "modifies" <+> pp n
